...
const ProductPage = () => {
  ...
  useEffect(() => {
    fetch(`https://my-json-server.typicode.com/kodplex/pr-re-ec-product/db`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    ...
    <div>
      <div>
        <p className=" focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 font-normal text-base leading-4 text-gray-600 mt-14">
      ...
      <div className="flex justify-center items-center lg:flex-row flex-col gap-8 mt-6">
        ...
        <div className=" w-full sm:w-96 md:w-8/12  lg:w-6/12 flex lg:flex-row flex-col lg:gap-8 sm:gap-6 gap-4">
          ...
          <div className=" w-full lg:w-4/12 grid lg:grid-cols-1 sm:grid-cols-4 grid-cols-2 gap-6">
            {productImage && productImage.map((img,index) => (
                <div className="bg-gray-100 flex justify-center items-center py-2 h-40">
                <img
                  src={img.image}
        ...
        <div className="  w-full sm:w-96 md:w-8/12 lg:w-6/12 items-center">
          ...
          <p className=" font-semibold lg:text-2xl text-xl lg:leading-6 leading-5 mt-6 ">
            $ {product.price} or {product.installment.amount}.00/{product.installment.period}
          </p>
          <p className=" font-normal text-base leading-5 text-gray-600 mt-2">
          ...
          <div className="lg:mt-5 mt-10">
            <div className="">
              ...
              <div className="flex gap-2">
                {productColors && productColors.map((color,index)=> (
                    <button className={`border rounded-full bg-[#${color}] p-2`} key={index}>.</button>
            ...
            <div className="flex mt-5 ml-1 ">
                               ...
                               <p className=" font-medium text-base leading-4 text-gray-600 px-2 w-40">
                                    Only <span className="text-orange-500">{product.stock} items</span> Left! Dont miss it
                                </p>
                            </div>
            <hr className=" bg-gray-200 w-full my-2" />
            <div className=" flex flex-row  items-center mt-4 gap-5">
             <button className="rounded-full bg-green-800 text-white p-2">Buy Now</button>
             <button className="rounded-full bg-slate-200 text-gray-500 p-2">Add to Cart</button>
                
            </div>
            <hr className=" bg-gray-200 w-full mt-4" />
          </div>

          <div className="focus:outline-none focus:ring-2 hover:bg-slate-200 focus:ring-offset-2 focus:ring-gray-800 font-medium text-base leading-4  w-full py-5 ">
